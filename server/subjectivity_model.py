...
df_tweet_input.loc[(df_tweet_input[TARGET_COLUMN] != 3),TARGET_COLUMN] = 0
df_tweet_input.loc[(df_tweet_input[TARGET_COLUMN] == 3),TARGET_COLUMN] = 1 #useless in this case
...
filename = 'finalized_countvectorizersubjectivity.sav'
dump(vct, filename)
...
dump(clf, 'subjectivity_model.joblib') 

titles_options = [("Confusion matrix, without normalization[SUBJECTIVITY]", None),
                  ("Normalized confusion matrix[SUBJECTIVITY]", 'true')]
