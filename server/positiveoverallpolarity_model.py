...
df_tweet_input.loc[(df_tweet_input[TARGET_COLUMN] != 4),TARGET_COLUMN] = 0
df_tweet_input.loc[(df_tweet_input[TARGET_COLUMN] == 4),TARGET_COLUMN] = 1 #useless in this case
...
filename = 'finalized_countvectorizerpositiveoverallpolarity.sav'
dump(vct, filename)
...
dump(clf, 'positiveoverallpolarity_model.joblib') 

titles_options = [("Confusion matrix, without normalization[POSITIVE_POLARITY]", None),
                  ("Normalized confusion matrix[POSITIVE_POLARITY]", 'true')]
