...
df_tweet_input.loc[(df_tweet_input[TARGET_COLUMN] != 5),TARGET_COLUMN] = 0
df_tweet_input.loc[(df_tweet_input[TARGET_COLUMN] == 5),TARGET_COLUMN] = 1 #useless in this case
...
filename = 'finalized_countvectorizernegativeoverallpolarity.sav'
dump(vct, filename)
...
dump(clf, 'negativeoverallpolarity_model.joblib') 

titles_options = [("Confusion matrix, without normalization[NEGATIVE_POLARITY]", None),
                  ("Normalized confusion matrix[NEGATIVE_POLARITY]", 'true')]
